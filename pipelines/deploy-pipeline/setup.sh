...
# Crete a DEV namespace for the application
 ...
 oc adm policy add-role-to-user -n springboot-camel-example-dev edit system:serviceaccount:$CURRENT_NAMESPACE:pipeline
 ...
 oc adm policy add-role-to-user -n springboot-camel-example-qa edit system:serviceaccount:$CURRENT_NAMESPACE:deploy_bot
 oc adm policy add-role-to-user -n springboot-camel-example-qa edit system:serviceaccount:$CURRENT_NAMESPACE:pipeline
 oc create secret docker-registry nexus-credentials --docker-server=nexus-container-registry-nexus.apps.cluster-3bcb.sandbox1869.opentlc.com --docker-username=admin --docker-password=admin123
